...
pub fn table_lookup_fixed_base(p: &mut PointPrecomp, digit: u64, sign: u64) {
    // SAFETY: scalar recoding constrains digit to the fixed table bounds; the
    // cast preserves the table's repr(C) point layout and destinations do not overlap it.
    unsafe {
...
pub fn montgomery_multiply_mod_order(ma: &[u64], mb: &[u64], mc: &mut [u64]) {
    ...
    // SAFETY: callers provide the fixed FourQ scalar widths (at least four
    // limbs, with an eight-limb product), and all copies are non-overlapping.
    unsafe {
...
pub fn r1_to_r3(p: &PointExtproj, q: &mut PointExtprojPrecomp) {
    ...
    // SAFETY: both repr(C) field elements contain exactly four contiguous u64
    // limbs and refer to distinct point values.
    unsafe {
        copy_nonoverlapping(p.z.as_ptr() as *mut u64, q.z2.as_mut_ptr() as *mut u64, 4)
        // ZQ = Z1
    }
}
...
pub fn r2_to_r4(p: &PointExtprojPrecomp, q: &mut PointExtproj) {
    ...
    // limbs and refer to distinct point values.
    unsafe {
        copy_nonoverlapping(p.z2.as_ptr() as *mut u64, q.z.as_mut_ptr() as *mut u64, 4)
        // ZQ = Z1
    }
}
...
pub fn point_setup(p: &PointAffine, q: &mut PointExtproj) {
    // SAFETY: every source and destination is a distinct fixed two-limb field
    // element in repr(C) point storage.
    unsafe {
...
pub fn ecc_mul_fixed(k: &[u64], q: &mut PointAffine) {
    ...
    // SAFETY: the scalar arrays have the fixed four-limb width required by
    // the recoder; the intrinsic-style helpers only access those limbs.
    unsafe {
        if scalar[0] & 1 == 0 {
...
pub fn ecc_mul_double(k: &mut [u64], l: &mut [u64], q: &mut PointAffine) -> bool {
    ...
    for i in (0..=64).rev() {
        ...
        // SAFETY: wNAF recoding bounds every non-zero digit to the
        // DOUBLE_SCALAR_TABLE range, whose repr(C) entries match PointPrecomp.
        unsafe {
            if digits_k1[i] < 0 {
                eccneg_precomp(
...
pub fn encode(p: &mut PointAffine, pencoded: &mut [u8]) {
    ...
    // SAFETY: callers provide at least 32 output bytes; PointAffine::y is a
    // contiguous 32-byte repr(C) value and cannot overlap the output.
    unsafe {
...
pub fn decode(pencoded: &[u8], p: &mut PointAffine) -> bool {
    ...
    // SAFETY: callers provide at least 32 encoded bytes; PointAffine::y is a
    // contiguous writable 32-byte repr(C) value and cannot overlap the input.
    unsafe {
